...
class AlpImport
{
    ...
    public enum DmdTypes : int {
        ...
        ALP_DMDTYPE_WQXGA_400MHZ_090A = 9,
        ALP_DMDTYPE_WQXGA_480MHZ_090A = 10,
	...
	public enum SeqTypes : int {
		...
		ALP_ON_TIME = 2214,
		ALP_OFF_TIME = 2215,
        // Frame Look Up Table (FLUT): sequence settings select how to use the FLUT. The look-up table itself is shared across all sequences. (use ALP_FLUT_SET_MEMORY controls for accessing it) */
        ALP_FLUT_MODE = 2118,	/* Select Frame LookUp Table usage mode: */
        ALP_FLUT_ENTRIES9 = 2120,	/* Determine number of FLUT entries; default=1 Entries: supports all values from 1 to ALP_FLUT_MAX_ENTRIES9 */
        ALP_FLUT_OFFSET9 = 2122,    /* Determine offset of FLUT index; default=0 Offset supports multiples of 256; For ALP_FLUT_18BIT, the effective index is half of the 9-bit index. --> "ALP_FLUT_ENTRIES18" and "ALP_FLUT_FRAME_OFFSET18" are 9-bit settings divided by 2 */
	...
	/// <summary> Use with ProjControlEx_Flut to write the Frame Look Up Table</summary>
	public enum FlutWriteModes : int
    {
        ALP_FLUT_WRITE_9BIT = 2325,	    /* 9-bit look-up table entries */
        ALP_FLUT_WRITE_18BIT = 2326,	/* 18-bit look-up table entries */
    }
    unsafe public struct tFlutWrite
    {
        ...
        public fixed UInt32 FrameNumbers[4096];
    };
	...
	/// <summary>
	/// General FrameLUT write function.
	/// ControlType selects numerical type of ControlValue.FrameNumbers.
	/// Call in "unsafe" context.
	/// </summary>
	/// <seealso cref="ProjControlEx_Flut9"/>
	/// <seealso cref="ProjControlEx_Flut18"/>
	public static Result ProjControlEx_Flut(UInt32 DeviceId, FlutWriteModes ControlType, ref tFlutWrite ControlValue)
    {
        unsafe
        {
            fixed (void* UserStructPtr = &ControlValue) {
                return (Result)AlpProjControlEx(DeviceId, (Int32)ControlType, UserStructPtr);
            }
        }
    }
	/// <summary>Specialized 18-bit FrameLUT write function</summary>
	public static Result ProjControlEx_Flut18(UInt32 DeviceId, UInt32 TableOffset, params UInt32[] FrameNumbers)
	{
		unsafe
		{
			AlpImport.tFlutWrite FrameLUT;
			FrameLUT.nOffset = TableOffset;
			FrameLUT.nSize = (uint) FrameNumbers.Length;
			if (FrameLUT.nOffset+FrameLUT.nSize > 2048) return Result.ALP_PARM_INVALID;	// limit: ALP_FLUT_MAX_ENTRIES9/2

			int i;
			for (i = 0; i < FrameLUT.nSize; i++) FrameLUT.FrameNumbers[i] = FrameNumbers[i];
			for (; i < 4096; i++) FrameLUT.FrameNumbers[i] = 0;

			return ProjControlEx_Flut(DeviceId, AlpImport.FlutWriteModes.ALP_FLUT_WRITE_18BIT, ref FrameLUT);
		}
	}
	/// <summary>Specialized 9-bit FrameLUT write function</summary>
	public static Result ProjControlEx_Flut9(UInt32 DeviceId, UInt32 TableOffset, params UInt32[] FrameNumbers)
	{
		unsafe
		{
			AlpImport.tFlutWrite FrameLUT;
			FrameLUT.nOffset = TableOffset;
			FrameLUT.nSize = (uint)FrameNumbers.Length;
			if (FrameLUT.nOffset + FrameLUT.nSize > 4096) return Result.ALP_PARM_INVALID;	// limit: ALP_FLUT_MAX_ENTRIES9

			int i;
			for (i = 0; i < FrameLUT.nSize; i++) FrameLUT.FrameNumbers[i] = FrameNumbers[i];
			for (; i < 4096; i++) FrameLUT.FrameNumbers[i] = 0;

			return ProjControlEx_Flut(DeviceId, AlpImport.FlutWriteModes.ALP_FLUT_WRITE_9BIT, ref FrameLUT);
		}
	}
    ...
    [DllImport(AlpDllName, CallingConvention = CallingConvention.Cdecl)]
    unsafe private static extern Int32 AlpProjControlEx(UInt32 DeviceId, Int32 ControlType, void *UserStructPtr);
    [DllImport(AlpDllName, CallingConvention = CallingConvention.Cdecl)]
